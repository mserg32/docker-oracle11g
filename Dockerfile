...
RUN curl -o /etc/pki/rpm-gpg/RPM-GPG-KEY-oracle -k https://public-yum.oracle.com/RPM-GPG-KEY-oracle-ol6
RUN curl -o /etc/yum.repos.d/public-yum-ol6.repo -O http://public-yum.oracle.com/public-yum-ol6.repo
RUN pwd; sed -i 's/enabled=0/enabled=1/' /etc/yum.repos.d/public-yum-ol6.repo
...
RUN groupadd oradba 
RUN groupadd oraoper
RUN useradd -g oinstall -G oradba oracle
...
RUN su - oracle; echo "export ORACLE_BASE=/opt/oracle" >> /home/oracle/.bash_profile \
...
RUN pwd; sed -i 's/enabled=1/enabled=0/' /etc/yum.repos.d/public-yum-ol6.repo
